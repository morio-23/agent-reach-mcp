    ...
    def _get_jwks_client(self) -> PyJWKClient:
        if self._jwks_client is None:
            self._jwks_client = PyJWKClient(
                self.jwks_url or _discover_jwks_url(self.issuer, self.timeout_seconds),
...
def build_auth(settings: Settings) -> tuple[TokenVerifier | None, AuthSettings | None]:
    ...
    if settings.auth_mode is AuthMode.STATIC_TOKEN:
        ...
        verifier = OAuthJWTVerifier(
            ...
            resource,
            settings.oauth_algorithms,
            settings.oauth_jwks_url,
        )
        issuer = settings.oauth_issuer
        required_scopes = settings.oauth_scopes
        # JWT verification above already validates the configured audience. Keep
        # MCP's RFC 8707 resource check disabled unless a future verifier derives
        # AccessToken.resource from an actual `resource` claim instead of the local
        # endpoint URL.
...
def _discover_jwks_url(issuer: str, timeout_seconds: float) -> str:
    request = urllib.request.Request(
        ...
        headers={"Accept": "application/json", "User-Agent": "agent-reach-mcp/0.1"},
    )
    with urllib.request.urlopen(request, timeout=timeout_seconds) as response:
        body = response.read(_DISCOVERY_MAX_BYTES + 1)
