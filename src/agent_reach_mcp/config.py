...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="AGENT_REACH_MCP_", env_file=".env", extra="ignore"
    )
    ...
    x_twifork_fallback_enabled: bool = True
    ...
    def validate_security(self):
        if self.auth_mode is AuthMode.STATIC_TOKEN and not self.static_token:
            raise ValueError(
                "static_token auth requires AGENT_REACH_MCP_STATIC_TOKEN"
            )
        if self.auth_mode is AuthMode.OAUTH:
            missing = [
                n
                for n, v in (
                    ("AGENT_REACH_MCP_OAUTH_ISSUER", self.oauth_issuer),
                    ("AGENT_REACH_MCP_OAUTH_AUDIENCE", self.oauth_audience),
                    ("AGENT_REACH_MCP_PUBLIC_BASE_URL", self.public_base_url),
                )
                if not v
            ]
            if missing:
        ...
        if (
            self.transport is TransportMode.STREAMABLE_HTTP
            and self.auth_mode is AuthMode.NONE
    ...
    def mcp_url(self) -> str:
        if self.public_base_url:
            return urljoin(
                self.public_base_url.rstrip("/") + "/", self.mcp_path.lstrip("/")
            )
