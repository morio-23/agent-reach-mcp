...
class Gateway:
    def __init__(self, settings: Settings):
        ...
        self._twitter = TwitterAdapter(
            ...
            twifork_fallback_enabled=settings.x_twifork_fallback_enabled,
            write_enabled=settings.x_write_enabled,
        )
    ...
    async def get_capabilities(self, refresh: bool = False) -> dict[str, Any]:
        ...
        if (
            ...
            and now - self._capabilities_cache[0]
            < self.settings.capabilities_cache_ttl_seconds
        ):
        ...
        async with self._capabilities_lock:
            ...
            if self.settings.x_write_enabled:
                tools.append("post_x")
            value = {
                "agent_reach": report,
                "mcp_tools": tools,
                "x": {
                    "primary_backend": "twitter-cli",
                    "twifork_fallback_enabled": self.settings.x_twifork_fallback_enabled,
                    "write_enabled": self.settings.x_write_enabled,
                    "write_backend": "twifork" if self.settings.x_write_enabled else None,
                },
            }
    ...
    async def read_url(
        self, url: str, max_chars: int | None = None
    ) -> dict[str, Any]:
        normalized = normalize_public_http_url(url)
        content = await asyncio.to_thread(self._web.read, normalized)
        limit = min(
            max_chars or self.settings.max_text_chars, self.settings.max_text_chars
        )
        if limit < 1000:
        ...
        return ReadUrlResult(
            ...
            content=content,
            truncated=truncated,
            warnings=["Content was truncated."] if truncated else [],
    ...
    async def search_x(
        ...
        return (
            await self._twitter.search(query, limit, from_user, since, until)
        ).model_dump(mode="json")
